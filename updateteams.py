...
def flairtext():
   ...
   for j in unique(fileteams.Conf):
    ...
    if len(confteams) > 0:
     ...
     for k in confteams.Filename:
      ...
      teamfull = teaminline+"|["+str(teamrow.Name[teamindex])+"](/message/compose/?to=cbbprivateflairmod&subject=flair&message="+nametourl+")"
 ...
 wikitext.write("\n\n*Created by /u/bakonydraco, available at /r/flairguide/wiki. Implemented on /r/cbbprivateflairtest by /u/ztrobin5.*")
 wikitext.close()
...
def replaceimage(folder, image):
 try:
  ...
  if(os.path.isfile(newimage)):
   ...
   if (imagesize > 512000):
    os.system("C:\\Users\\Owner\\Documents\\Flairguide\\flairguide-master\\teamsheet\\optipng.exe " + newimage)
   if (imagesize > 512000):
    print "Imagesize = ",imagesize
    print "Running pngquant:"
    os.system('C:\\Users\\Owner\\Documents\\Flairguide\\flairguide-master\\teamsheet\\pngquant.exe --ext=.png --force ' + newimage)
    # os.system("C:\\Users\\Owner\\Documents\\Flairguide\\flairguide-master\\teamsheet\\pngquant.exe teamsheet/new"+i.lower()+".png --ext=.png --force")
    print "pngquant complete"
    print "Imagesize = ",imagesize
...
def updatewikis():
 ...
 try:
  print "Attempting to update flair text page on reddit..."
  sub = r.get_subreddit(subreddit)
  sub.edit_wiki_page(cbbprivateflairtest,"flair", wikitextheader + wikitext, reason='')
  time.sleep(2)
