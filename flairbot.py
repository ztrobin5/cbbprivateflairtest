...
username = "cbbprivateflairmod"
password = "CBBprivateflairmod!!"
subreddit = "cbbprivateflairtest"
pausetime = 10
robotmessage = "\n\n*I'm just a robot and this account is not checked often, so if you have further questions please check our [FAQ](/r/cbbprivateflairtest/wiki/rules)*."
modlist = ['cbbprivateflairmod']
...
def checkmessages(r):
 for message in r.get_unread(limit=None):
  print message
  auth = str(message.author).encode("utf8")
  subj = message.subject.encode("utf8").strip().lower().replace("&amp;","&")
  body = message.body.encode("utf8").replace("&amp;","&")
  if (subj == "flair"): setflair(auth,body,r)
  elif (subj == "flairuser"): setflairuser(auth,body,r)
  elif (subj == "award"): logaward(auth,body,message,r)
  elif (subj == "grantaward"): grantaward(auth,body,message,r)
...
def setflairuser(auth,body,r):
 if auth in modlist["cbbprivateflairtest"]:
  auth = body.split("\n")[0]
  bodyuser = "\n".join(body.split("\n")[1:]).rstrip()
  setflair(auth,bodyuser,message,subredditflag,r)
 else: 
...
awardtables = ["","","/r/cbbprivateflairtest Emeritus Mod","/r/cbbprivateflairtest Artist","/r/cbbprivateflairtest Patron"]
def checkaward(auth,award,r):
 ...
 awardswiki = r.get_wiki_page("cbbprivateflairtest","awards")
...
def logaward(auth,body,message,r):
 try:
  sub = r.get_subreddit('cbbprivateflairtest')
...
def grantaward(auth,body,message,r):
 if auth in modlist["cbbprivateflairtest"]:
  try: 
   authuser = body.split("\n")[0].strip()
   bodyuser = body.split("\n")[1].strip()
   ...
   time.sleep(2)
   r.send_message(authuser,'You won a ' + bodyuser + ' award flair!', "Check out our [Awards Page](/r/cbbprivateflairtest/wiki/awards) for details on how you won.  You can now change to and from it at any time from the [flair selector](/r/cbbprivateflairtest/wiki/flair)."+robotmessage)
  except: print "Error granting award"
 else: 
  r.send_message(auth, 'Permission Denied', "Sorry, you don't have permission to set images for that subreddit."+robotmessage)
  print "Permission Denied: " + subreddits 

print "cbbprivateflairtestFlair start time: " + strftime("%Y-%m-%d %H:%M:%S", gmtime())
